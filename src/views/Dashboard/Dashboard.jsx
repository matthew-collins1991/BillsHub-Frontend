...
import {
  returnSeries,
  GetMonthBillCostsAndLabels,
  GetYearBillCostsAndLabels,
  GetNextMonthBillCostsAndLabels
} from "../../variables/DateSort";
...
class Dashboard extends React.Component {
  ...
  allUtilitiesArray = () => {
    let allUtilities = [];
    this.props.userInfo.utilities &&
      this.props.userInfo.utilities.map(
        utility => (allUtilities = [...allUtilities, utility])
      );
    return allUtilities;
  };

  getMonthlyBillLabels = () => {
    return GetMonthBillCostsAndLabels(this.allUtilitiesArray()).map(
      bill => bill.utility_type
    );
  };

  getMonthlyBillCosts = () => {
    return [
      GetMonthBillCostsAndLabels(this.allUtilitiesArray()).map(
        bill => bill.cost
      )
    ];
  };
  ...
  getYearlyBillLabels = () => {
    return GetYearBillCostsAndLabels(this.allUtilitiesArray()).map(
      bill => bill.utility_type
    );
  };
  ...
  getYearlyBillCosts = () => {
    return [
      GetYearBillCostsAndLabels(this.allUtilitiesArray()).map(bill => bill.cost)
    ];
  };
  ...
  getTotalMonthlyOutgoings = () => {
    let totalCost = 0;
    GetMonthBillCostsAndLabels(this.allUtilitiesArray()).map(
      bill => (totalCost = totalCost + bill.cost)
    );
  ...
  getTotalNextMonthlyOutgoings = () => {
    let totalCost = 0;
    GetNextMonthBillCostsAndLabels(this.allUtilitiesArray()).map(
      bill => (totalCost = totalCost + bill.cost)
    );
    return parseFloat(Math.round(totalCost * 100) / 100).toFixed(2);
  };
  ...
  allBillsArray = () => {
    let allBills = [];
    this.props.userInfo.utilities &&
      this.props.userInfo.utilities.map(utility =>
            ...
            <Card>
              <CardHeader color="warning" stats icon>
                ...
                <p className={classes.cardCategory}>
                  Your total monthly outgoings for {ThisMonth()}:
                </p>
              ...
              <CardFooter stats>
                <div className={classes.stats}>
                  <Warning />
                  <p>
                    Your predicted outgoings for next month is £
                    {this.getTotalNextMonthlyOutgoings()}
                  </p>
                </div>
              </CardFooter>
            </Card>
            ...
            <Card chart>
              <CardHeader color="info">
                <ChartistGraph
                  ...
                  responsiveOptions={
                    MonthlyBillBarChart(this).responsiveOptions
                  }
              ...
              <CardBody>
                <h4 className={classes.cardTitle}>
                  Total payments by utility type for {ThisMonth()}
                </h4>
              </CardBody>
              <CardFooter chart>
                <div className={classes.stats}>
            ...
            <Card>
              <CardHeader color="danger" icon>
                ...
                <h4 className={classes.cardIconTitle}>
                  Distribution of payments for {ThisMonth()}:
                </h4>
              </CardHeader>
              <CardBody>
                <ChartistGraph
          ...
          <GridItem xs={12} sm={12} md={5}>
            <Card>
              <CardHeader color="danger" icon>
                ...
                <h4 className={classes.cardIconTitle}>
                  Distribution of payments for {this.thisYear()}:
                </h4>
              </CardHeader>
              <CardBody>
                <ChartistGraph
